...
class Actor:
    ...
    def use_brain(
        self, inputs, explore=False
    ):  # inputs can be a tuple of controls or the full input for the brain
        if self.brain != None:
            if not explore:
    ...
    def compare_positions(
        self,
    ):  # distance between current position and oldest stored position
        return math.sqrt(
            (self.x - self.comp_positions[0][0]) ** 2
            + (self.y - self.comp_positions[0][1]) ** 2
        )

    def compare_angles(self):  # distance between current and oldest stored angle
        return (
            min(
                abs(self.angle_deg - self.comp_angles[0]),
                360 - abs(self.angle_deg - self.comp_angles[0]),
            )
            / 360
        )
    ...
    def compare_distances(
        self,
    ):  # distance between current position and oldest stored distance
        return self.comp_distance[0] - self.comp_distance[-1]

    def compare_smooth_distances(
        self,
    ):  # distance between current position and oldest stored distance
