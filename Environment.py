...
class Environment(gym.Env):
    def __init__(
        self, display_sim=True, brain=None, max_time=1024, tile_matrix=np.array([])
    ):
        pygame.init()
        if tile_matrix.any():
        ...
        self.observation_space = gym.spaces.Box(
            low=0, high=100, shape=(self.map.get_space_size() + 6,), dtype=np.float32
        )
    ...
    def get_reward(self):
        on_target, min_distance, direction = self.map.target_intersections(
            self.actor.x, self.actor.y
        )
        self.actor.comp_distance.append(
            self.map.distance_map[int(self.actor.y // 100)][int(self.actor.x // 100)]
        )
    ...
    def get_state(self):
        ...
        state = np.concatenate(
            (
                self.map.get_local_map(self.actor.x, self.actor.y, 3).flatten(),
                np.array(self.actor.get_state()),
            )
        )
    ...
    def step(self):
        ...
        if self.display_sim:
            ...
            self.display.render(
                self.entities,
                self.map.get_local_map(self.actor.x, self.actor.y, 3),
                [
    ...
    def collision_detection(self):
        ...
        collision, center, dx, dy = self.boundary_detection(
            [self.actor.x, self.actor.y],
            self.actor.radius,
            self.actor.dx,
            self.actor.dy,
    ...
    def predict_actor_collision(self, wall):
        collision_x, center_x, dx, _ = self.circle_rect_collision_x(
            [self.actor.x, self.actor.y],
            self.actor.radius,
            wall.get_rect(),
            self.actor.dx,
        ...
        collision_y, center_y, _, dy = self.circle_rect_collision_y(
            [self.actor.x, self.actor.y],
            self.actor.radius,
            wall.get_rect(),
            self.actor.dx,
    ...
    def circle_rect_collision_x(self, circle_center, radius, rect, dx, dy):
        ...
        closest_point = (
            max(rect.left, min(circle_center[0], rect.right)),
        ...
        if (
            rect.top < circle_center[1] - radius
            and rect.bottom > circle_center[1] - radius
        ) or (
            rect.top < circle_center[1] + radius
            and rect.bottom > circle_center[1] + radius
        ):
    ...
    def circle_rect_collision_y(self, circle_center, radius, rect, dx, dy):
        ...
        if (
            rect.left < circle_center[0] - radius
            and rect.right > circle_center[0] - radius
        ) or (
            rect.left < circle_center[0] + radius
            and rect.right > circle_center[0] + radius
        ):
