...
from MapRLlib import *
from ActorRLlib import *
...
class Environment(gym.Env):
    def __init__(self, env_config={}):
        ...
        self.FPS = 60
        self.max_time = 1024
        self.max_stall_time = 150
        self.max_episode_steps = 1024

        # Define action and observation space
        self.action_space = gym.spaces.Box(low=-1, high=1, shape=(3,), dtype=np.float32)
        self.observation_space = gym.spaces.Box(low=-10, high=10, shape=(self.map.get_minimap_size() + self.actor.get_state_size() + 1,), dtype=np.float32)
    ...
    def reset(self, *, seed=None, options=None):
        ...
        #new_map = self.map.generate_random_tilemap(len(self.map.tile_matrix), len(self.map.tile_matrix[0]))
        #self.map = Map(tile_matrix=new_map)
    ...
    def get_reward(self): 
        ...
        reward -= 0.4 * self.actor.time_still/(self.max_stall_time) #between -0.5 and 0
    ...
    def get_state(self):
        ...
        #right direction is the actors little cheat code
        ...
        env_state = self.map.get_local_map(self.actor.x, self.actor.y).flatten()
    ...
    def step(self, action):
        ...
        self.event_handler(action) #This is the main difference. model is external, rather than held in actor.brain
    ...
    def render(self, render_mode = "human"):
        if render_mode == "human":
            ...
            _, reward = self.get_state(), self.get_reward()
            ...
            if self.state_type == "sensor":
                self.display.render(self.entities, self.map.get_local_map(self.actor.x, self.actor.y), \
    ...
    def event_handler(self, action):
        ...
        if self.run_mode == "human":
            self.actor.apply_inputs(pressed[K_w], pressed[K_a], pressed[K_d])
        else:
            self.actor.apply_inputs(action[0], action[1], action[2])
